...
export async function handleCommit() {
  try {
    ...
    if (!config) {
      console.error(
        "❌ Hata: .gitsafe.yml bulunamadı. Lütfen önce `gitsafe init` komutunu çalıştırın."
      );
      process.exit(1);
    }
    ...
    if (!(await hasStagedFiles())) {
      console.log(
        "ℹ️ Commitlemek için hazırlanmış (staged) hiçbir değişiklik bulunamadı."
      );
      const { proceedToAdd } = await inquirer.prompt([
        {
          ...
          name: "proceedToAdd",
          message:
            "`gitsafe add` yardımcısını çalıştırarak şimdi dosya eklemek ister misiniz?",
          default: true,
        },
      ]);
    ...
    console.log(
      "\n🔍 Commit'e dahil edilecek değişiklikler gözden geçiriliyor..."
    );
    ...
    if (added.length > 0) {
      summaryText +=
        `\n  Eklenenler (${added.length}):\n` +
        added.map((f) => `    + ${f.path}`).join("\n");
    }
    if (modified.length > 0) {
      summaryText +=
        `\n  Değiştirilenler (${modified.length}):\n` +
        modified.map((f) => `    M ${f.path}`).join("\n");
    }
    if (deleted.length > 0) {
      summaryText +=
        `\n  Silinenler (${deleted.length}):\n` +
        deleted.map((f) => `    - ${f.path}`).join("\n");
    }
    if (renamed.length > 0) {
      summaryText +=
        `\n  Yeniden Adlandırılanlar (${renamed.length}):\n` +
        renamed.map((f) => `    R ${f.path}`).join("\n");
    }

    console.log(summaryText);

    const { proceedToCommit } = await inquirer.prompt([
      {
        type: "confirm",
        name: "proceedToCommit",
        message: `\nBu ${stagedFiles.length} değişikliği commitlemek istediğinize emin misiniz?`,
        default: true,
      },
    ]);
    ...
    for (const sub of stagedSubmodules) {
      ...
      if (!isPushed) {
        ...
        console.warn(
          `Bu commit (${sub.newCommit.substring(
            0,
            7
          )}) uzak depoda bulunmuyor ve bu durum takım arkadaşlarınızın projesini kıracaktır.`
        );
        ...
        const { action } = await inquirer.prompt([
          {
            ...
            choices: [
              {
                name: `'${sub.path}' submodule'ünü şimdi push'la ve commite devam et (Tavsiye Edilir)`,
                value: "push",
              },
    ...
    if (gitError.stderr) {
      ...
      console.error(
        "\n❌ Commit işlemi sırasında bilinmeyen bir hata oluştu.",
        error
      );
    }
  }
