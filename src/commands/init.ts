  ...
  const answers = await inquirer.prompt([
    {
      ...
      name: "commitMessage",
      message:
        "Commit mesajları için bir standart zorunlu kılınsın mı? (Örn: feat: ...)",
      choices: [
        {
          name: "Evet, Conventional Commits standardı zorunlu olsun.",
          value: "conventional",
        },
    ...
    },
    {
      ...
      choices: [
        {
          name: "Strict: Standart dışı isimleri engelle (örn: feature/login)",
          value: "strict",
        },
    ...
    },
    {
      ...
      name: "mainBranch",
      message:
        "Projenin ana branch'inin adı nedir? (örn: main, master, production)",
      default: "main",
    },
    {
      type: "confirm",
      ...
      message:
        "Doğrudan push'a karşı korumalı branch'ler belirlemek ister misiniz?",
      default: true,
    },
    {
      type: "input",
      ...
      message:
        "Korunacak branch'leri yazın (virgülle ayırın, boş bırakmak için Enter):",
    ...
    },
    {
      ...
      name: "setupHusky",
      message:
        "Husky ile Git hook'ları otomatik olarak kurulsun mu? (Tavsiye Edilir)",
      default: true,
    },
    {
      type: "checkbox",
      name: "hooksToInstall",
      message:
        "Hangi gitsafe otomasyonlarını (Git Hooks) aktive etmek istersiniz?",
...
protectedBranches: []`;
  ...
  if (answers.setupProtectedBranches && answers.protectedBranchesInput) {
    const branches = answers.protectedBranchesInput
      .split(",")
      .map((b: string) => b.trim())
      .filter((b: string) => b); // Boş girdileri temizle
...
`;
  ...
  if (
    answers.setupHusky &&
    answers.hooksToInstall &&
    answers.hooksToInstall.length > 0
  ) {
    try {
      ...
      if (answers.hooksToInstall.includes("pre-push")) {
        ...
        const prePushScriptContent = `npx --no-install gitsafe validate-push`;
      ...
      if (answers.hooksToInstall.includes("commit-msg")) {
        ...
        const commitMsgScriptContent = `npx --no-install gitsafe validate-commit "$1"`;
      ...
      console.error(
        "Lütfen projenizde Node.js ve npm'in kurulu olduğundan ve package.json dosyanızın olduğundan emin olun."
      );
    }
